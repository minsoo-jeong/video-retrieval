...
def evaluate_fivr5k(model, loader, epoch, args):
    ...
    for k, x, l in loader:
        ...
        features[k[0]] = base_model.forward_once(x.cuda()).cpu().numpy()
...
class VideoTriplet(nn.Module):
    def __init__(self, frame_encoder: nn.Module, video_encoder: nn.Module):
        super(VideoTriplet, self).__init__()
        self.frame_encoder = frame_encoder
        self.video_encoder = video_encoder
        self.freeze_frames_encoder()
    ...
    def freeze_frames_encoder(self):
        self.frame_encoder.requires_grad_(False)
...
class VCDBDistractionTwoTransformDataset(Dataset):
    def __init__(self, index, length=64, transform1=None, transform2=None):
        self.vcdb = VCDB()
        self.index = load_pickle(index)
        self.length = length
    ...
    def __getitem__(self, idx):
        ...
        t1 = apply_albumentations_for_video(x, self.transform1)
        t2 = apply_albumentations_for_video(x, self.transform2)
...
def main(args):
    ...
    model = VideoTriplet(Resnet50(), SimpleMLP(2048, 2048)).to(args.device)

    fivr_dataset = FIVRDataset('/workspace/fivr5k_frames.pkl', version='5k', transform=default_transform)
    train_dataset = VCDBTripletDataset('/workspace/vcdb_frames.pkl', length=32,
    ...
    criterion = torch.nn.TripletMarginWithDistanceLoss(margin=0.3,

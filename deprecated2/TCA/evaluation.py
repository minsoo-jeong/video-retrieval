...
def calculate_similarities(query_features, target_feature, metric='euclidean', comparator=None):
    ...
    similarities = []
    if metric == 'euclidean':
        dist = np.nan_to_num(cdist(query_features, target_feature, metric='euclidean'))
        for i, v in enumerate(query_features):
            sim = np.round(1 - dist[i] / dist.max(), decimals=6)
            similarities.append(sim.item())
    elif metric == 'cosine':
        dist = np.nan_to_num(cdist(query_features, target_feature, metric='cosine'))
        for i, v in enumerate(query_features):
            sim = 1 - dist[i]
            similarities.append(sim.item())
    elif metric == 'chamfer':
...
def query_vs_database(model, dataset, args):
    ...
    test_loader = DataLoader(FeatureDataset(vid2features,
    ...
    for feature, feature_len, video_id in tqdm(test_loader):
        ...
        if feature.shape[1] > 0:
            if args.cuda:
                ...
                feature_len = feature_len.cuda()
            if args.metric == 'chamfer':
                embedding = model.encode(feature, feature_len).detach().cpu().numpy()[0]
            else:
...
class Basic(torch.nn.Module):
    ...
    def forward(self, x, num_frames):
        ...
        frame_mask = (0 < num_frames - torch.arange(0, T).cuda()).float()
        frame_count = torch.sum(frame_mask, dim=-1, keepdim=True)  # (N, 1)

        output = torch.sum(x, dim=2) / frame_count  # (N, C)
    ...
    def encode(self, x, num_frames):
        N, C, T = x.shape
        if len(num_frames.shape) == 1:
            num_frames = num_frames.unsqueeze(1)  # (N, 1)
        frame_mask = (0 < num_frames - torch.arange(0, T).cuda()).unsqueeze(-1)  # (N, T, 1)
...
def main():
    ...
    parser.add_argument('-mp', '--model_path', type=str,
                        required=False,
                        default='/mlsun/ms/tca/ckpts/resnet50-rmac/model_ep_0.pth',
    ...
    parser.add_argument('-m', '--metric', type=str, default='chamfer',

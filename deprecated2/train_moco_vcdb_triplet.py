...
from futures.moco import MoCo_Triplet
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    train_dataset = VideoDataset_VCDBTriplet(vcdb=VCDB(),
                                             negative=4,
                                             index='data/vcdb-10k.json',
                                             frames_per_clip=1,
                                             sampling_rate=1,
                                             sampling_unit='fps',
    ...
    model = MoCo_Triplet(encoder, dim=2048, K=4096, m=0.999, T=0.07).cuda()
    if args.distributed:
...
def train(model, loader, criterion, optimizer, scheduler, epoch, args):
    ...
    for idx, (_, _, _, a, p, n, len_a, len_p, len_n) in enumerate(loader, start=1):
        logits, labels = model(a.cuda(), p.cuda(), n.cuda(), len_a.cuda(), len_p.cuda(), len_n.cuda())
